...
public final class ProtectedScreenOverlay {
    ...
    public static void register() {
        ScreenEvent.Render.Post.BUS.addListener(ProtectedScreenOverlay::onRender);
        ScreenEvent.MouseButtonPressed.Pre.BUS.addListener((java.util.function.Predicate<ScreenEvent.MouseButtonPressed.Pre>) ProtectedScreenOverlay::onMouseClick);
        ScreenEvent.KeyPressed.Pre.BUS.addListener((java.util.function.Predicate<ScreenEvent.KeyPressed.Pre>) ProtectedScreenOverlay::onKeyPressed);
        ScreenEvent.Closing.BUS.addListener(ProtectedScreenOverlay::onClosing);
    }
    ...
    private static boolean onMouseClick(ScreenEvent.MouseButtonPressed.Pre e) {
        if (!ProtectedSlotsCache.isMarkMode()) return false;
        if (!(e.getScreen() instanceof AbstractContainerScreen<?> screen)) return false;
        if (e.getInfo().button() != 0) return false;
        Slot hovered = findHovered(screen, e.getMouseX(), e.getMouseY());
        if (hovered == null) return false;
        if (!(hovered.container instanceof Inventory)) return false;
    ...
    public static boolean attemptToggle(int idx) {
        ...
        MSNetwork.CHANNEL.send(
            new UpdateProtectedSlotsPacket(ProtectedSlotsCache.toArray()),
            PacketDistributor.SERVER.noArg()
        );
        return true;
    }

    private static boolean onKeyPressed(ScreenEvent.KeyPressed.Pre e) {
        if (suppressHotkey) return false;
        if (!(e.getScreen() instanceof AbstractContainerScreen<?> screen)) return false;
        if (MagicStorageClient.TOGGLE_PROTECT.matches(e.getInfo())) {
